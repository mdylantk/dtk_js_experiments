	...
	var CanvasGNGin = { //js canvas base functions
		...
		MouseClick : function(x,y){ //currently set up to draw square where a click happen
			...
			dType={type:this.drawType,left:tX,top:tY,width:32,height:32,color:"green",strokeColor:"red",style:this.styleType,radius:16,sAngle:0,eAngle:Math.PI*2,cClock:false,text:":3",font:"Arial",size:32,format:"center"}; //note color is not needed. last used color will be appled base on type. it just here for more control and proably is best to have this handle it. Also the drawType should be save to the cell for redraws and modifications
			//note: size may be scaled base on grid size. idk i want to keep px and scale up/down if needed.
			//note: size for text may be an issue if using long strings. may need a autoscale to keep it from overfilling capped zones 
			
			var cellName = "x="+Math.round(x/32)+":y="+Math.round(y/32);
			if (this.mainGrid.cells[cellName] == undefined) //draw it once
			{
				...
				this.mainGrid.cells[cellName] = dType;


			}
			else //temp remove option
			{
				delete this.mainGrid.cells[cellName];
			}
			this.Redraw(this.mainGrid.cells);
			
			
			
			//this.Ctx.beginPath();
			//this.Ctx.rect(x-16, y-16, 32, 32); //left, top, w, h
		...
		Redraw : function(renders) { //draw all var in object. var type should be correct render type
			...
			var x;
			var error = {}; //this is not effecient in defugging failed draws. it just a temp option
			for (x in renders){
		...
		Init : function(CanvasName) { //set the canvas data from a element name
			...
			//this.elem.onclick = function() { CanvasGNGin.MouseClick(80,80);};
			this.elemLeft = this.elem.offsetLeft;
			this.elemTop = this.elem.offsetTop;
			this.Canvas.addEventListener('click', function() { 
			CanvasGNGin.MouseX = event.pageX - CanvasGNGin.elemLeft;
			CanvasGNGin.MouseY = event.pageY - CanvasGNGin.elemTop;
			CanvasGNGin.MouseIsClick = true;
			CanvasGNGin.MouseClick(event.pageX - CanvasGNGin.elemLeft,event.pageY - CanvasGNGin.elemTop); }, false);
